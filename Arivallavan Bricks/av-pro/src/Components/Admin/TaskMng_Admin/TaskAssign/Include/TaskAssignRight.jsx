...
const handleMultipleSelected = async () => {
  const selectedTasks = filteredTasks.filter(task =>
    selectedTaskKeys.includes(`${task.FinNo}_${task.TaskId}`)
  );

  if (selectedTasks.length === 0) {
    showPopup('No tasks selected.', 'error');
    return;
  }
  ...
  try {
    const enrichedTasks = await Promise.all(
      selectedTasks.map(async (task) => {
        // Fetch Department and TaskDescription from TaskAssignedDetails
        const res = await axios.get(`http://localhost:3001/taskassigneddetails/${task.TaskId}`);
    ...
    await axios.post('http://localhost:3001/taskstatusdetails/multiple', enrichedTasks);
    showPopup('Tasks submitted successfully.', 'success');
  } catch (error) {
    console.error('Error submitting tasks:', error);
    showPopup('Failed to submit selected tasks.', 'error');
  }
};
  return (
    <>
                  ...
                  <div className="card">
                    ...
                    <div className="card-body">
                      ...
                      <div className=" text-end">
                        ...
                        <button className="btn btn-primary" onClick={handleMultipleSelected}>
  Multiple Selected
</button>
                        </div>
